 ...
 */
 ...
 FROM EMPLOYEE
 GROUP BY DEPT_CODE
 ORDER BY DEPT_CODE NULLS FIRST; -- NULLS FIRST 를 쓸때에는 칼럼명을 명시해줘야 함 숫자로 쓰면 안됨 
 ...
 /*
  ...
  - 그룹에 대한 조건을 제시할 때 사용하는 구문
   ...
   6 : ORDER BY  컬러명 | 별칭 | 컬럼순번 [ASC|DESC] [NULLS FIRST|NULLS LAST]
 */
 
 -- 부서별 평균 급여가 300만원 이상인 직원의 평균 급여를 조회(부서코드, 평균급여)
 SELECT DEPT_CODE,  TO_CHAR(ROUND(AVG(NVL(SALARY,0))), 'FM999,999,999') "평균급여"
 FROM EMPLOYEE
 GROUP BY DEPT_CODE
...
/*  
*데이터베이스 기술 관련 질문에 많이 나옴/ 현장에선 많이 안씀

   집계 함수
   - 그룹별 산출한 결과 값의 중간 집계를 계산 해주는 함수
   - ROLLUP(컬럼1, 컬럼2) : 컬럼 1을 가지고 다시 중간집계를 내는 함수
   - CUBE(컬럼1, 컬럼2) : 컬럼 1을 가지고 중간집계도 내고, 컬럼2를 가지고 중간집계를 또 냄
...
/*
 GROUPING
 - ROLLUP 이나 CUBE에 의해 산출된 값이 해당 컬럼의 집합의 산출물이면
...
-- 위 쿼리문 대신 WHERE 절에 OR 연산자를 사용해서 쿼리 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
