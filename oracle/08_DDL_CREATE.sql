 ...
 */
 ...
 /*
   ...
   * 테이블 생성
   - 테이블이란? 행(ROW)과 열(COLLUM)로 구성되는 가장 기본적인 데이터베이스 객체
 ...
 */
 ...
 /*
   ...
   - 사용자가 객체를 생성하거나 객체를 변경하는 등의 작업을 할 때 데이터베이스
     서버에 의해서 자동으로 갱신되는 테이블
 */
 
 --1. USER_TABLES : 사용자가 가지고 있는 테이블들의 전반적인 구조를 확인할 수 있는 시스템 테이블
 SELECT * FROM USER_TABLES;
 
 SELECT*
 FROM USER_TABLES
 WHERE TABLE_NAME = 'MEMBER'; --여러 테이블중 하나를 가져 올 때
 
 --2. USER_TAB_COLUMNS : 사용자가 가지고 있는 테이블들 상의 모든 컬럼의 전반적인 구조를 
 --                       확인할 수 있는 시스템 테이블
 ...
 */
 ...
 INSERT INTO MEMBER VALUES(1,'user01', 'pass01', '홍길동', '남', 
                            '010-1111-2222', 'aaa@naver.com','23/06/26');
...
SELECT * FROM MEMBER; 
 ...
 /*
    * 제약조건(CONSTRAINTS)
    - 사용자가 원하는 조건의 데이터만 유지하기 위해서 각 컬럼에 대해 저장될 
      값에 대한 제약조건을 설정
...
INSERT INTO MEM_CHECK VALUES(1,'user01','pass01','홍길동',null,null,null); 
...
INSERT INTO MEM_PRI VALUES(null,'user02','pass02','이순신','남',null,null);
 --> 기본키 null을 담으려고 할때 (not null 제약조건 위배)
 --> PRIMARY KEY 제약조건을 부여하면 그 컬럼에 자동으로 NOT NULL + UNIQUE 제약조건이 설정
...
INSERT INTO MEM_PRI2 VALUES(1,'user02','pass01','홍길녀',null,null,null);
-- PRIMARY KEY(MEM_NO,MEM_ID)로 키값을 두개 묶었기 때문 1 값이 똑같아도 'user01' 이게 달라서 들어감
...
INSERT INTO TB_LIKE VALUES (1,'A',SYSDATE); -- 똑같은 상품 또 구매시 : 오류남 //복합키 추천 X
...
--> 유효한 회원등급번호가 아님에도 불구하고 INSERT! 아직 연결은 안된것
SELECT*FROM MEM;
...
/*
  * FOREIGN KEY(외래키) 제약조건
  ...
  --> 참조할컬럼명 생략 시 참조할테이블명에 PRIMARY KEY로 지정된 컬럼으로 매칭
...
CREATE TABLE MEM(
   ...
   GRADE_ID NUMBER REFERENCES MEM_GRADE--(GRADE_CODE) -->컬럼레벨 방식
                                                  --GRADE_ID 와 GRADE_CODE를 일치시킴 
                                                  --(GRADE_CODE)생략가능-PRIMARY KEY 라서 생략가능
...
--> 이때 부모테이블(MEM_GRADE)에서 데이터값을 삭제할 경우 문제 발생!
   ...
   DELETE FROM MEM_GRADE
   WHERE GRADE_CODE = 30; --> 삭제 됨
 --> 자식테이블이 이미 사용하고 있는 값이 잇을 경우
...
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
   FROM EMPLOYEE
   WHERE 1 =0;  --> 구조만 복사하고자 할때 이 조건 사용(데이터 값은 안나옴 구조만 나옴)
...
CREATE TABLE TB_BOOK(
   ...
   BK_AUTHOR VARCHAR2(40) NOT NULL,
   BK_PRICE NUMBER,
...
INSERT INTO TB_BOOK VALUES(4,'혼자 공부하는 얄팍한 코딩 지식','고현민',16200,1);
...
INSERT INTO TB_MEMBER VALUES(3,'USER3','1234','강호동','M','서울시 강남구','010-5555-6666','N',DEFAULT);
...
--6.
SELECT BK_TITLE, PUB_NAME, RENT_DATE +7
FROM TB_BOOK B
JOIN TB_PUBLISHER P ON (P.PUB_NO = B.BK_PUB_NO)
JOIN TB_RENT R ON(R.RENT_BOOK_NO = B.BK_NO)
JOIN TB_MEMBER M ON(M.MEMBER_NO = R.RENT_MEM_NO )
WHERE M.MEMBER_NO = 1;
...
/*
   DB 모델링 작업 순서
    ...
    2. 논리적 모델링
      - 속성 추출
      - 정규화 작업(1,2,3) - 정규화 작업은 3까지만 추천,,, // 역정규화도 있음
    3. 물리적 모델링
      - 테이블 실질적으로 작성
