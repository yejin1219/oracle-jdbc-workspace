 ...
 */
 ...
 INSERT INTO EMP_01
 VALUES(200,'이병헌','개발 자원팀'); --값의 수가 충분하지 않습니다
 
 -- 에러 발생하지 않지만 데이터가 잘못 저장(순서 중요!)
 INSERT INTO EMP_01
 VALUES(300,'유아지원팀','공유',DEFAULT);
 
 -- 컬럼순서와 데이터 타입이 맞지 않아서 에러 발생
 INSERT INTO EMP_01
 VALUES('유아지원팀',300,'위하준',DEFAULT); --수치가 부적합합니다
 ...
 -- EMP_NAME 컬럼에 NOTNULL 제약조건에 의해서 에러 발생
 INSERT INTO EMP_01(EMP_ID,DEPT_TITLE) -- NAME에는 필수로 값이 들어가야 함
 VALUES(700,'마케팅팀'); 
 ...
 INSERT INTO EMP_01( -- ** 테이블 컬럼의 개수와 데이터 타입이 같아야 함
 ...
 */
 ...
 UPDATE EMP_SALARY
 SET DEPT_CODE = 'D0' -- FOREIN KEY 제약조건에 위배된다. ?????
 WHERE EMP_NAME = '노옹철';
 ...
 -- 방명수 사원의 급여와 보너스를 유재식 사원과 동일하게 변경
  --1) 단일행 서브쿼리를 각각의 컬럼에 사용
   UPDATE EMP_SALARY
   SET SALARY = (SELECT SALARY FROM EMP_SALARY WHERE EMP_NAME = '유재식'),
       BONUS = (SELECT BONUS FROM EMP_SALARY WHERE EMP_NAME = '유재식')
   WHERE EMP_NAME = '방명수';
 
  --2) 다중열 서브쿼리를 사용해서 SALARY, BONUS 컬럼들 한 번에 번경!
   UPDATE EMP_SALARY
 ...
 /*
   DELETE : 테이블에 기록된 데이터를 삭제하는 구문(행 단위로 삭제)
 ...
 */
 ...
 /*
    TRUNCATE : 테이블의 전체 행을 삭제할 때 사용하는 구문
     ...
     - 별도의 조건을 제시할 수 없고 ROLLBACK 이 불가능하다.
 ...
 */
 ...
 -- 롤백 시 복구 불가능
 TRUNCATE TABLE DEPT_COPY;-- 테이블은 남아 있음
 TRUNCATE TABLE EMP_SALARY;

 DROP TABLE DEPT_COPY; -- 테이블 모두 지워짐 
