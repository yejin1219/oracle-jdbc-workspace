 ...
 */
 ...
 /*
    ...
    * 캐시 메모리
      ...
      - 매번 호출될 때마다 새로이 번호를 생성하는게 아니라 캐시메모리 공간에 미리 
        생성된 값들을 가져다 쓸 수 있음 (속도가 빨라짐)
      - 접속이 해제되면 => 캐시메모리에 미리 만들어 준 번호들은 다 날라감
 ...
 */
 ...
 SELECT SEQ_EMPID.NEXTVAL FROM DUAL; --300 ( START WITH 300으로 지정했기 때문)
 SELECT SEQ_EMPID.CURRVAL FROM DUAL; --300
 
 SELECT SEQ_EMPID.NEXTVAL FROM DUAL; --305
 SELECT SEQ_EMPID.NEXTVAL FROM DUAL; --310
 
 SELECT SEQ_EMPID.NEXTVAL FROM DUAL; -- 에러발생.MAXVALUE 310이기 때문 초과 됨
...
/*
    3. 시퀀스 구조 변경
     ...
     [MAXVALUE 숫자]  --> 최대값 지정 (기본값 무진장 큼)
     [MINVALUE 숫자]  --> 최소값 지정 (기본값 1)
     [CYCLE|NOCYCLE]  --> 값 순환 여부 비정 (기본값 NOCYCLE)
     [NOCACHE|CACHE 바이트크기] --> 캐시메모리 할당 (기본값 CACHE 20)
