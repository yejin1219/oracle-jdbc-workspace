/*
  PL / SQL (PROCEDURE LANGUAGE EXTENSION YO SQL)
   ...
   * PL / SQL 구조
     - [선언부(DECLARE SECTION)] : DECLEAR 로 시작, 변수나 상수를 선언 및 초기화하는 부분
...
/*
  1-1) 일반 타입 변수 선언 및 초기화
      [표현식]
      변수명 [CONSTANT] 자료형 [:=값]; --CONSTANT 넣으면 상수 없으면 변수
...
BEGIN SELECT EMP_ID, EMP_NAME, JOB_CODE,DEPT_TITLE,SALARY
      INTO EID, ENAME, JCODE, DTITLE, SALARY
      FROM EMPLOYEE E
...
-- NCODE 값이 KO일 경우 => TEAM 에 '국내팀' 대입,
--  그게 아닐 경우 => TEAM 에 '해외팀' 대입
-- 사번(EID), 이름(ENAME), 부서(DTILTE), 소속(TEAAM)에 대해 출력
DECLARE
...
*/
-- 사용자에게 점수를 입력받아 SCORE(데이터타입 : NUMBER) 변수에 저장한 후 학점은 
...
-- D7인 경우 해외엽업3부, D8인 경우 기술지원부, D9인 경우 총무부
-- 나머지는 부서없음
...
BEGIN
   LOOP DBMS_OUTPUT.PUT_LINE(NUM);
        NUM := NUM + 1;
        IF(NUM > 5) THEN EXIT; --1) IF 조건식 THEN EXIT; END IF;
        END IF;
   END LOOP;     
END;
/
...
BEGIN
    LOOP DBMS_OUTPUT.PUT_LINE(NUM);
         NUM := NUM + 1;
         EXIT WHEN NUM>5; -- 2) EXIT WHEN 조건식;
     END LOOP;
END;
/
...
FOR NUM IN 2..9 
   LOOP
    ...
    FOR SU IN 1..9 
    LOOP
    DBMS_OUTPUT.PUT_LINE(NUM || '*' || SU || '=' || NUM*SU);
   END LOOP;
...
BEGIN DBMS_OUTPUT.PUT_LINE(10/&숫자);
EXCEPTION
