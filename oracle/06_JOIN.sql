 ...
 */
 ...
 -- 사원명, 부서명, 급여, 연봉 조회
    ...
    -- *** 부서 배치가 아직안 된 사원 2명에 대한 정보는 조회 X
 ...
 -- 3) RIGHT [OUTER] JOIN : 두 테이블이 가진 모든 행을 조회할 수 있음
      -- >> ANSI 구문
 SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
 FROM EMPLOYEE
 FULL JOIN DEPARTMENT ON( DEPT_CODE = DEPT_ID);
    ...
    -- >> 오라클 구문 (에러 발생! 오라클 구문으로는 FULL JOIN 사용X)
 SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
 FROM EMPLOYEE, DEPARTMENT
 WHERE DEPT_CODE(+) = DEPT_ID(+); --(+)는 하나만 지정 가능
 ...
 */
 ...
 WHERE E.MANAGER_ID = M.EMP_ID(+); --(+) 붙여서 사수가 없는 사람들도 출력
 ...
 */
     -- >> ANSI 구문
 SELECT EMP_NAME, DEPT_TITLE  -- 사원: 23 부서 :7 개
 FROM EMPLOYEE
 CROSS JOIN DEPARTMENT; -- 23X7 = 107개 나옴     (잘 안씀...)
...
SELECT EMP_NAME, BONUS, TO_CHAR(SALARY*12,'FM999,999,999'), DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
WHERE BONUS IS NOT NULL;
 ...
 SELECT DEPT_TITLE,TO_CHAR( ROUND(AVG(NVL(SALARY,0))),'FM999,999,999')
 FROM EMPLOYEE
 LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
 GROUP BY DEPT_TITLE
 ORDER BY DEPT_TITLE;
 ...
 SELECT DEPT_TITLE, TO_CHAR(SUM(SALARY), 'FM999,999,999')
 FROM EMPLOYEE E
 JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID)
 GROUP BY DEPT_TITLE
